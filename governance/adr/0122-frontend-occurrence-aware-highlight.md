...
**Negatywne / koszt**: PDF occurrence best-effort przez granice ekstrakcji (mitygowane clamp-to-first). Lokator nie jest persystowany -> po reload brak occurrence (fallback). Logika wyboru wystapienia operuje na 30-znakowym prefiksie segmentu (jak istniejacy matcher) - przyblizenie wystarczajace przy unikalnym prefiksie frazy.
