...
- koszt: migracja 015 ALTER CHECK (w publicznej linii; 010-014 zajete) + lustro w schema.sql i schema.sqlite.ts (konwencja jak 005/007/008/009).
...
- `backend/src/lib/audit.ts`, `schema.sqlite.ts`, `schema.sql`, `migrations/015_audit_log_event_type_cost_cap.sql` - whitelist event_type `cost_cap`.
