...
**Status**: Przyjety - ship ZA FLAGA (2026-06-29, akceptacja WM). Wzorzec jak ADR-0101/0102 (Konstytucja v1.6.0/v1.6.1): implementacja wjezdza za flaga `PATRON_MUTATION_APPROVAL` (domyslnie OFF = zero zmiany zachowania), a **FLIP flagi na ON oraz merge do `main` pozostaja bramkowane: review round 2 + eval** (Sec 6.1). Review round 1 PRZESZEDL (matematic-patron-pr-review-pl na pelnym diffie Phase 2-4: 0 blockerow; 3 should-fix naprawione - parytet edits staged/inline, RODO art. 17/20 dla `mutation_approvals` w rodo-delete/rodo-export; mikro-race approve w trybie serwerowym udokumentowany, desktop single-user niewrazliwy). Konstytucja bump 1.7.0 (MINOR - nowy wpis audit_log `mutation.approval.decision`, Sec 6.1). US1+US2 zaimplementowane i zademonstrowane E2E. Dotyka sciezki wykonania narzedzi (tool-dispatch) i audit_log. Inspiracja architektoniczna: open-mercato (MIT, `prepareMutation` + `confirm-required` + pending approval + approval-card), zaadaptowana do prymitywow PATRONa - NIE port kodu.
