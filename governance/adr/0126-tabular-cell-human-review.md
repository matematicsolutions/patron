...
**Status**: Proponowany 2026-06-14. Konstytucja v1.5.0. Wybor WM 2026-06-14 (T2.2 z backlogu OpenContracts). Implementacja ETAPOWA: **12a** czysty model `cell-review.ts` (state machine approved/rejected/corrected + efektywna tresc) ZROBIONE; **12b** persystencja DUAL-MODE ZROBIONE 2026-07-04 (kolumny review w `tabular_cells`: SQLite `schema.sqlite.ts` + idempotentne ALTER w `ensureSchemaUpgrades`; Postgres `schema.sql` + migracja `migrations/018_tabular_cell_review.sql` UP/DOWN) - port WIP z worktree oc na release/v2.0.0-prep; **12c** route ZROBIONE 2026-07-04 jako `POST /tabular-review/:reviewId/cells/review` (body: document_id + column_index + action, parytet identyfikacji z regenerate-cell zamiast :cellId w path; requireAuth + ensureReviewAccess; store przez shim `db.from`), kontrolka w UI komorki (TRTable/TRSidePanel) = REZERWACJA. Stacked na ADR-0125 (branch feat/oc-locator-loose-match).
