...
- **NIE propagujemy decyzji Gateway'a do audit hash-chain (ADR-0001)** - na razie `console.warn`. Propagacja do `audit_log` (faktyczna nazwa tabeli, `backend/schema.sql:397`) z polem `event_type = "mcp_security.gateway"` w przyszlym ADR (gdy bedzie UI dla Operatora do zatwierdzania `human_review`).
  - **Errata 2026-05-27**: pierwotnie ADR-0028 mowil `audit_events`. Tabela faktycznie nazywa sie `audit_log` (od ADR-0001). ADR-0033 (propagacja MCP Security do audit) wdrazany 2026-05-24 wpiety juz prawidlowo do `audit_log`. Ten zapis zostawiony bez zmiany historycznej tresci poza dorzucona errata - kanon ADR (immutability historycznych zapisow).
