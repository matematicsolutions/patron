...
**Status**: Zaproponowany 2026-06-03. Konstytucja zaktualizowana do v1.6.0 (Art. 5 - mechanizm zgody Operatora). **NIE merge bez 2x wewnetrznego review (WM) + re-podpisu Konstytucji przez kancelarie**. Zweryfikowany na zywej binarce: sprawa oznaczona `attorney_client_privileged` + model chmurowy odpowiada bez blokady, bez zadnych flag w konsoli (zgoda wbudowana w `main.js`); decyzja egress trafia do audytu z reason `privileged-cloud-by-operator`.

**Data**: 2026-06-03

**Powiazane zasady** (Konstytucja AI Patrona v1.6.0):
- **Art. 5 - Tajemnica zawodowa** (+ **Art. 1 - Lokalnosc danych**): OBA artykuly juz dopuszczaja egress "za swiadoma decyzja osoby uprawnionej" (Art. 5 odsyla do def. w § 4; Art. 1: "Patron nie ma trybu, w ktorym tresc dokumentu trafia do zewnetrznego operatora bez swiadomej decyzji"). To KOD (twardy blok `decideRoute` bez wyjatku) byl OSTRZEJSZY niz Konstytucja. Ten ADR dostraja kod do zasady: w trybie desktop single-user adwokat JEST osoba uprawniona na wlasnej maszynie, a jego wybor modelu chmurowego jest ta swiadoma decyzja. Granica governance to "audyt + maskowanie PII + decyzja czlowieka", nie "blokada wyboru modelu". Doprecyzowane w Art. 5 (bullet "zgoda Operatora", v1.6.0).
...
- **Zrobione**: doprecyzowanie **Konstytucji Art. 5** (bullet "zgoda Operatora", v1.6.0) + wpis w changelogu - kod zgodny z dokumentem. **WARUNEK merge**: re-podpis Konstytucji przez kancelarie (zmiana MINOR dotyka egzekwowania tajemnicy). Rozwazyc widoczna w UI informacje "ta sprawa przetwarzana w chmurze za Twoja zgoda" + ewentualny przelacznik per-sprawa.
...
- [x] Doprecyzowanie Konstytucji Art. 5 + changelog (v1.6.0).
- [x] Pelny `vitest run` backend: 1095 pass / 5 todo / 0 fail (78 plikow).
