...
> Dokument governance opisujący zasady, ograniczenia i role
> obowiązujące w pracy z produktem **Patron** — lokalnym agentem AI
> dla polskich kancelarii prawnych. Pisany w polskim porządku prawnym
> z uwzględnieniem AI Act, RODO i Zasad Etyki Adwokackiej / Zasad
> Etyki Zawodowej Radcy Prawnego.
...
**Patron to lokalny agent AI dla polskiej kancelarii prawnej**, który
pomaga prawnikowi pracować szybciej z dokumentami i orzecznictwem,
**zachowując tajemnicę zawodową, RODO i przewidywalność**.
...
klienta ma znacznik `[N]` + blok `<CITATIONS>` z dokładnym fragmentem
...
nie opuszczają infrastruktury kancelarii **bez świadomej decyzji
osoby uprawnionej do podejmowania decyzji compliance**. Definicja
"świadomej decyzji" → patrz § 4.
...
sięga w danej turze czatu — nie cały folder klienta.

Konektory MCP (SAOS/NSA/ISAP/EUR-Lex) odpytują **publiczne źródła
prawa** — nie ma w nich danych klienta kancelarii.
...
kancelaria może wykonać audyt bezpieczeństwa przez własny zespół IT lub
...
- **5 konektorów MCP** (`mcp-saos`, `mcp-nsa`, `mcp-isap`, `mcp-krs`,
  `mcp-eu-sparql` — osobne repo) — **MIT**. To infrastruktura do
  publicznych źródeł prawa polskiego — chcemy aby cały ekosystem
  polskiego legal-techu mógł je wpinać do swoich produktów,
  niezależnie od ich licencji.
...
| **Nie ukrywa wywołań LLM ani konektorów MCP** | Wszystko widoczne w audit log + UI (panel cytatów). |
...
| **Nie używa danych klienta do trenowania modeli MateMatic** | Klauzula umowna + brak technicznych ścieżek wysyłki. |
| **Nie ingeruje w wybór modelu kancelarii** | Bring-your-own-model. Patron jest powłoką. |
| **Nie tworzy reprezentatywnych zbioru przykładów dla marketing/sales** | Bez explicit pisemnej zgody klienta kancelarii. |
...
Odpowiada za:
- Comiesięczny przegląd `audit_log` (sample 10% wpisów + verify chain).
- Aktualizację rejestru czynności przetwarzania (RODO art. 30).
- Decyzję o eksporcie / usunięciu danych klienta (RODO art. 17 + 20).
...
- **Wniosek RODO art. 17 (prawo do bycia zapomnianym)**: zachowujemy
  audit_log (compliance obligation > prawo do usunięcia), ale anonimizujemy
  pole `actor_user_id` przez ustawienie na NULL (FK `ON DELETE SET NULL`).
...
Czego audyt **nie pokrywa**: treści dokumentów klientów (te są w
`document_versions` / MinIO i mają osobny cykl życia + szyfrowanie
...
- **MAJOR** (X.0.0): zmiana znaczenia zasady, usunięcie zasady,
  zwężenie roli, łamanie kontraktu API. Wymaga pisemnej akceptacji
  Administratora i okresu przejściowego ≥ 60 dni.
- **MINOR** (1.X.0): nowa zasada, rozszerzenie roli, nowy wpis
  w audit_log. Wymaga notatki dla Administratora przed wdrożeniem.
- **PATCH** (1.0.X): doprecyzowanie, literówka, korekta linku.
  Wystarcza commit + changelog.
...
- **Konektor MCP** — osobny proces (`mcp-saos`, `mcp-nsa`, `mcp-isap`,
  `mcp-eu-sparql`) wystawiający narzędzia (`saos__search`, `nsa__get_judgment`,
  `isap__get_act`, `eu-sparql__search_by_celex` itd.).
- **Self-host** — instalacja produktu na infrastrukturze klienta;
  dane nie opuszczają serwera kancelarii.
