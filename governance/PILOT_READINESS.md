...
1. **Pakowanie instalatora desktop (electron-builder)** - **ZAMKNIETE 2026-06-01 (ADR-0091, main d97e648, zweryfikowane wizualnie na Windows).** Instalator NSIS dziala: Electron bundled node (ELECTRON_RUN_AS_NODE) + standalone frontend server.js + electron-rebuild better-sqlite3 pod ABI + prepare-resources staging. Instalacje u Testera przeprowadza Operator OSOBISCIE na miejscu (uprawnienia administratora od dzialu IT klienta) - rozwiazuje brak praw admina u testerow IT-managed. Ponizej oryginalny (historyczny) opis gapa sprzed zamkniecia: jedyny zidentyfikowany realny gap kodowy dla sciezki DESKTOP. `desktop/main.js` w produkcji odpala `npm run dev` (serwer deweloperski Next.js) i `node dist/index.js`, a `desktop/package.json` extraResources WYKLUCZA `node_modules` i `src` z obu paczek. Skutek: spakowany instalator dalby aplikacje, ktora nie wstaje (brak natywnego better-sqlite3 w backendzie, brak zrodel/zaleznosci frontu). Dodatkowo main.js spawnuje zewnetrzny `node`/`npm`, co wymaga toolchainu Node na maszynie klienta - sprzeczne z celem "jeden instalator". Wymaga przeprojektowania: serwowanie frontu produkcyjnie (standalone server.js) zamiast dev, bundling produkcyjnych zaleznosci backendu (w tym rebuild natywny better-sqlite3 pod runtime), uruchamianie procesow przez wbudowany node Electrona zamiast zewnetrznego. Weryfikacja wymaga zbudowania NSIS i instalacji na czystej maszynie Windows (GUI) - poza srodowiskiem agenta headless. Status: wydzielone do osobnego zadania.
...
6. **Disclosure: baza nieszyfrowana w spoczynku w wersji pilotazowej.** Szyfrowanie at-rest (ADR-0072) odroczone na wersje po-sprzedazowa (decyzja Wieslawa 2026-06-01). W pilotazu lokalna baza jest plaintext na dysku - MUSI byc jawnie zakomunikowane Testerowi PRZED instalacja (Pouczenie RODO, dok. 2 pakietu pilotazowego, rozdz. 1 + potwierdzenie w Protokole przekazania, dok. 3) z zaleceniem szyfrowania dysku (BitLocker), szczegolnie na urzadzeniu bez szyfrowania dysku. Operator/Tester decyduje o zabezpieczeniu urzadzenia i ponosi odpowiedzialnosc.
...
- Szyfrowanie bazy at-rest default-on (ADR-0072): odroczone na wersje po pilotazu; w pilotazu baza plaintext (patrz brama ludzka nr 6 - wymagany disclosure).
