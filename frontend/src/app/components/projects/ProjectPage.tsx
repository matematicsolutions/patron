...
export function ProjectPage({ projectId, initialTab = "documents" }: Props) {
    ...
    const [folders, setFolders] = useState<PATRONFolder[]>([]);
    const [chats, setChats] = useState<PATRONChat[]>([]);
    ...
    const [viewingDoc, setViewingDoc] = useState<PATRONDocument | null>(null);
    ...
    const [versionsByDocId, setVersionsByDocId] = useState<
        Map<string, PATRONDocumentVersion[]>
    >(() => new Map());
     ...
     */
    function handleUploadNewVersion(doc: PATRONDocument) {
    ...
    useEffect(() => {
        Promise.all([
            ...
            listProjectChats(projectId).catch(() => [] as PATRONChat[]),
    ...
    function handleDocsSelected(newDocs: PATRONDocument[]) {
        setProject((prev) =>
            prev ? {
                ...prev,
                documents: [
    ...
    async function handleDeleteChatRow(chat: PATRONChat) {
    ...
    function hasMovePayload(dt: DataTransfer): boolean {
        return Array.from(dt.types).some(
            (type) =>
                type === "application/patron-doc" ||
                type === "application/patron-folder",
        );
    }
    ...
    async function handleDropOnFolder(targetFolderId: string | null, dt: DataTransfer) {
        ...
        const docId = dt.getData("application/patron-doc");
        const subFolderId = dt.getData("application/patron-folder");
        if (docId) {
            const doc = (project?.documents ?? []).find((d) => d.id === docId);
