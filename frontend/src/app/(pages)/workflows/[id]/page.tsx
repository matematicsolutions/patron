...
import { getWorkflow, updateWorkflow } from "@/app/lib/patronApi";
...
export default function WorkflowDetailPage({ params }: Props) {
    ...
    const [workflow, setWorkflow] = useState<PATRONWorkflow | null>(null);
    ...
    if (notFound || !workflow) {
        return (
            <div className="flex-1 flex items-center justify-center">
                <p className="text-gray-400 font-serif">{t("workflows.notFound")}</p>
            </div>
        );
    }
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between px-8 py-4 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-gray-400">
                        {saveStatus === "saving"
                            ? t("workflows.savingStatus")
                            : saveStatus === "saved"
                              ? t("workflows.savedStatus")
                              : ""}
                    </span>
                    ...
                    {canShare && (
                        <button
                            ...
                            aria-label={t("workflows.openWorkflowPeople")}
                            title={t("workflows.people")}
            ...
            {readOnly && (
                <div className="flex items-center h-10 px-8 border-b border-gray-200">
                    <span className="text-xs text-gray-400">{t("workflows.readOnly")}</span>
                </div>
            )}
                    ...
                    <div className="flex flex-col flex-1 min-h-0">
                        ...
                        <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium shrink-0 select-none">
                            ...
                            <div className={`sticky left-8 z-[60] ${NAME_COL_W} bg-white pl-2 text-left`}>
                                {t("workflows.columnTitle")}
                            </div>
                            <div className="ml-auto w-36 shrink-0">{t("workflows.format")}</div>
                            <div className="flex-1 min-w-0">{t("workflows.prompt")}</div>
                            ...
                            {columns.length === 0 ? (
                                <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                    ...
                                    <p className="text-2xl font-medium font-serif text-gray-900">
                                        {t("workflows.columnsEmptyTitle")}
                                    </p>
                                    <p className="mt-1 text-xs text-gray-400 text-left">
                                        {t("workflows.columnsEmptyBody")}
                                    </p>
                                    {!readOnly && (
                                        <button
                                            onClick={() => setAddColumnOpen(true)}
